...
public class InitBean {
	...
	public void init() {
		...
		try {
			...
			PatientDto david = patientFactory.createPatientDto();
			david.setName("David Doe");
			david.setDob(LocalDate.parse("1995-08-15"));

			david.setId(patientService.addPatient(david));
			
			ProviderDto Provider2 = providerFactory.createProviderDto();
			Provider2.setName("Donald");
			Provider2.setNpi("1122");
			...
			radio01.setDiagnosis("Cancer_treatment");
			radio01.setPatientName(david.getName());
			radio01.setProviderName(Provider2.getName());
			radio01.setPatientId(david.getId());
			radio01.setProviderId(Provider2.getId());
			ArrayList<LocalDate> radio_dates = new ArrayList<>();
			radio_dates.add(LocalDate.parse("2022-12-22"));
			radio01.setTreatmentDates(radio_dates);

			
			PhysiotherapyTreatmentDto physio01 = treatmentFactory.createPhysiotherapyTreatmentDto();
			physio01.setDiagnosis("mental illness");
			physio01.setPatientId(david.getId());
			physio01.setPatientName(david.getName());
			physio01.setProviderId(Provider2.getId());
			physio01.setProviderName(Provider2.getName());
			ArrayList<LocalDate> physio_dates = new ArrayList<>();
			physio_dates.add(LocalDate.parse("2022-12-26"));
			physio01.setTreatmentDates(physio_dates);
			providerService.addTreatment(physio01);
			ArrayList<TreatmentDto> treatment_collection = new ArrayList<>();
			treatment_collection.add(physio01);
			radio01.setFollowupTreatments(treatment_collection);
			providerService.addTreatment(radio01);

			PatientDto Patient3 = patientFactory.createPatientDto();
			Patient3.setName("Patrick");
			Patient3.setDob(LocalDate.parse("1999-10-23"));
			Patient3.setId(patientService.addPatient(Patient3));
			
			ProviderDto Provider3 = providerFactory.createProviderDto();
			Provider3.setName("Peter");
			Provider3.setNpi("4321");
			...
			surgery01.setDiagnosis("Broken_Hand");
			surgery01.setDischargeInstructions("Your hand will be in a plaster");
			surgery01.setPatientId(Patient3.getId());
			surgery01.setPatientName(Patient3.getName());
			surgery01.setProviderId(Provider3.getId());
			surgery01.setProviderName(Provider3.getName());
			surgery01.setSurgeryDate(LocalDate.parse("2015-06-30"));
